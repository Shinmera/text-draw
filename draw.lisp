...
(defun horizontal-line (width height &key stream (bend :middle) arrow)
  (with-normalized-stream (stream stream)
    (when (< width 0) (setf width (- width) height (- height)))
    (cond ((< 1 width)
           (ecase arrow ((:start :end) (decf width 1)) ((T) (decf width 2)) ((NIL)))
           (let* ((l (ecase bend
                       ((:left :start) 1)
                       ((:right :end) (- width 2))
                  ...
                  (lpad l) (rpad r))
             (cond ((< +1 height)
                    (case arrow ((T :start) (incf lpad)))
                    (case arrow ((T :end) (incf rpad)))
                    (format stream "~@[~a~]~v{─~}╮~v{ ~}~%" (case arrow ((T :start) (arrow :left))) l 0 rpad 0)
                    (dotimes (i (- height 2))
                      (format stream "~v{ ~}│~v{ ~}~%" lpad 0 rpad 0))
                    (format stream "~v{ ~}╰~v{─~}~@[~a~]" lpad 0 r 0 (case arrow ((T :end) (arrow :right)))))
                   ((< height -1)
                    (case arrow ((T :end) (incf lpad)))
                    (case arrow ((T :start) (incf rpad)))
                    (format stream "~v{ ~}╭~v{─~}~@[~a~]~%" lpad 0 r 0 (case arrow ((T :start) (arrow :right))))
                    (dotimes (i (- (- height) 2))
                      (format stream "~v{ ~}│~v{ ~}~%" lpad 0 rpad 0))
                    (format stream "~@[~a~]~v{─~}╯~v{ ~}" (case arrow ((T :end) (arrow :left))) l 0 rpad 0))
                   (T
                    (case arrow ((T :start) (format stream "~a" (arrow :left))))
                    (format stream "~v{─~}" width 0)
                    (case arrow ((T :end) (format stream "~a" (arrow :right))))))))
          ((= 1 (abs height))
           (format stream "~a~%" (if arrow (arrow :left-right) "─")))
          ((= 1 width)
           (if (< 0 height)
               (format stream "╮~v{~%│~}~%╰~%" (- height 2) 0)
               (format stream "╭~v{~%│~}~%╯~%" (- (- height) 2) 0))))))

(defun vertical-line (width height &key stream (bend :middle) arrow)
  (with-normalized-stream (stream stream)
    (when (< height 0) (setf width (- width) height (- height)))
    (cond ((< 1 height)
           (ecase arrow ((:start :end) (decf height 1)) ((T) (decf height 2)) ((NIL)))
           (let* ((u (ecase bend
                       ((:top :start) 1)
                       ((:bottom :end) (- height 2))
                  ...
                  (b (- height u 1)))
             (cond ((< +1 width)
                    (case arrow ((T :start) (format stream "~a~v{ ~}~%" (arrow :up) (- width 1) 0)))
                    (dotimes (i u)
                      (format stream "│~v{ ~}~%" (- width 1) 0))
                    (format stream "╰~v{─~}╮~%" (- width 2) 0)
                    (dotimes (i b)
                      (format stream "~v{ ~}│~%" (- width 1) 0))
                    (case arrow ((T :end) (format stream "~v{ ~}~a~%" (- width 1) 0 (arrow :down)))))
                   ((< width -1)
                    (case arrow ((T :start) (format stream "~v{ ~}~a~%" (- (- width) 1) 0 (arrow :up))))
                    (dotimes (i u)
                      (format stream "~v{ ~}│~%" (- (- width) 1) 0))
                    (format stream "╭~v{─~}╯~%" (- (- width) 2) 0)
                    (dotimes (i b)
                      (format stream "│~v{ ~}~%" (- (- width) 1) 0))
                    (case arrow ((T :end) (format stream "~a~v{ ~}~%" (arrow :down) (- (- width) 1) 0))))
                   (T
                    (case arrow ((T :start) (format stream "~a~%" (arrow :up))))
                    (format stream "~v{│~^~%~}" height 0)
                    (case arrow ((T :end) (format stream "~a~%" (arrow :down))))))))
          ((= 1 (abs width))
           (format stream "~a~%" (if arrow (arrow :up-down) "│")))
          ((= 1 height)
           (if (< 0 width)
               (format stream "╰~v{─~}╮~%" (- width 2) 0)
               (format stream "╭~v{─~}╯~%" (- (- width) 2) 0))))))
